...
class Callback(ABC):
    ...
    def on_epoch_start(self, exp: Experiment) -> None:
        pass

    def on_epoch_end(self, exp: Experiment) -> None:
        pass

    def on_train_start(self, exp: Experiment) -> None:
        pass

    def on_train_end(self, exp: Experiment) -> None:
        pass

    def on_train_step_start(self, exp: Experiment) -> None:
        pass

    def on_train_step_end(self, exp: Experiment) -> None:
        pass

    def on_valid_start(self, exp: Experiment) -> None:
        pass
    
    def on_valid_end(self, exp: Experiment) -> None:
        pass
    ...
    def on_valid_end(self, exp: Experiment) -> None:
        ...
        if self._is_better(exp.metric[self.metric_name]):
            ...
            if exp.accelerator.is_main_process:
                # 兼容DDP和非DDP
                model_to_save = getattr(exp.model, "module", exp.model)
                torch.save(
                    model_to_save.state_dict(),
                    os.path.join(exp.config["folder"], f"model_{exp.step}.pth"),
                )
...
class WarmUpSchedule(Callback):
    ...
    def on_train_start(self, exp: Experiment) -> None:
        total_step: int = len(exp.train_dataloader) * exp.config["epoch"]
